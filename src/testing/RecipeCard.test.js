  ...
  test("formats ingredients correctly", () => {
    ...
    const formattedIngredients = mockRecipe.ingredients
      ...
      .map((ingredient) =>
        ingredient
          .replace(
            /^\d*\s*(шт\.?|г|кг|л|мл|ст\.л.|ч\.л.|додати|паст[аи]|см\.|капсула|таблетка|упаковка|щіпка|банка|пучок|-|зубчик[аи])?\s*/i,
            ""
          )
          .trim()
      )
      .join(", ");
    expect(
      screen.getByText(new RegExp(formattedIngredients, "i"))
    ).toBeInTheDocument();
  });
