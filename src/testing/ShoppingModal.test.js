...
import ShoppingListModal from "../components/RecipeDetailComponents/ShoppingListModal";
...
describe("ShoppingListModal", () => {
  ...
  it("should render the modal with initial ingredients", () => {
    render(
      <ShoppingListModal ingredients={initialIngredients} onClose={mockClose} />
    );
  ...
  it("should add a new ingredient to the list", () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Введіть інгредієнт"), {
      target: { value: "Tomato" },
    });
    fireEvent.keyDown(screen.getByPlaceholderText("Введіть інгредієнт"), {
      key: "Enter",
    });
...
*/
  it('should show input field when "Add Ingredient" is clicked', () => {
    ...
    expect(
      screen.getByPlaceholderText("Введіть інгредієнт")
    ).toBeInTheDocument();
  });
});
